...
function buildTable(data, frameCount) {
  ...
  headRow.innerHTML = `<th>Step</th><th>Page</th>${frameHeaders}<th>Status</th><th>Replaced</th><th>Action</th>`;
  ...
  data.steps.forEach((step, idx) => {
    const normalizedFrames = Array.from(
      { length: frameCount },
      (_, i) => step.frames?.[i] ?? null,
    );
    const replacedPage = step.replaced ?? step.evicted ?? null;
    const hitIndex = Number.isInteger(step.hitIdx) ? step.hitIdx : -1;
    const replaceIndex = Number.isInteger(step.replaceIdx)
      ? step.replaceIdx
      : -1;
    const actionText = step.reason
      ? step.reason
      : step.fault
        ? replacedPage !== null
          ? `Replaced page ${replacedPage} with ${step.page}`
          : "Loaded into empty frame"
        : `Hit in F${hitIndex + 1}`;

    const tr = document.createElement("tr");
    tr.id = `row-${idx}`;
    const cells = normalizedFrames
      .map((val, i) => {
        const isHit = !step.fault && hitIndex === i;
        const isFault = step.fault && replaceIndex === i;
        return `<td style="${isHit ? "color:var(--green)" : isFault ? "color:var(--red)" : ""}">${val ?? "—"}</td>`;
      })
      .join("");

    tr.innerHTML = `
      <td style="color:var(--text-muted)">${step.step ?? idx + 1}</td>
      <td class="cell-page">${step.page}</td>
      ${cells}
      <td class="${step.fault ? "cell-fault" : "cell-hit"}">${step.fault ? "✗ FAULT" : "✓ HIT"}</td>
      <td style="color:var(--text-dim)">${replacedPage ?? "—"}</td>
      <td style="text-align:left;color:var(--text-dim);font-size:12px;max-width:260px">${actionText}</td>`;
...
function showStep(idx) {
  ...
  document.querySelectorAll(".timeline-item").forEach((el, i) => {
    ...
    if (i < idx)
      el.classList.add("done", App.simData.steps[i].fault ? "fault" : "hit");
    else if (i === idx) {
      el.classList.add("current");
      if (tlScroll)
        tlScroll.scrollLeft = Math.max(
          0,
          el.offsetLeft - tlScroll.offsetWidth / 2 + el.offsetWidth / 2,
        );
    }
  });
  ...
  for (let i = 0; i < App.simData.frameCount; i++) {
    ...
    if (val === null) box.classList.add("frame-empty");
      ...
      box.classList.add(step.replaced !== null ? "replaced" : "fault");
  }
...
function renderReasoning(step) {
  ...
  step.frames.forEach((f, fi) => {
    ...
    const evictTag = isEvicted
      ? '<span style="opacity:.7">← evicted</span>'
      : "";

    if (isLRU && step.lruAges) {
      const age = step.lruAges[fi];
      const ageStr =
        age === 0 ? "just used" : `${age} step${age !== 1 ? "s" : ""} ago`;
...
function toggleTable() {
  ...
  document.getElementById(
    "tableBody",
  ).parentElement.parentElement.style.display = App.tableVisible
    ? "block"
    : "none";
}
